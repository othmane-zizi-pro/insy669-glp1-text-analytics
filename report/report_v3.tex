...
\section{Methodology}
\textbf{Collection strategy and data types.} Public data were collected from Reddit (community posts) and WebMD (patient reviews), while media data were collected from Google News RSS with GLP-1-specific queries and optional article-body expansion. This yields mixed text types that reflect real consumption contexts: short conversational posts (Reddit), semi-structured patient narratives (WebMD), and publisher-formatted news coverage.

\textbf{Information retrieval tools and rationale.} Reddit collection used Arctic Shift endpoints; WebMD used deterministic HTML scraping; media collection used RSS retrieval, Google link decoding, and bounded HTML body extraction. This stack was selected because it is reproducible with open tools (\texttt{requests}, \texttt{BeautifulSoup}), does not require proprietary access, and supports source traceability from raw collection to processed corpus.

\textbf{Text analytics methods and rationale.} Preprocessing used tokenization, lowercasing, stopword removal, and lemmatization to standardize cross-source language. VADER was used for sentiment because it is robust for short informal text and still interpretable on news prose. TF-IDF and Bag-of-Words were used for lexical comparison and features because they are transparent and suitable for domain terms. Supervised separability was tested with Multinomial Naive Bayes and KNN as complementary baselines (probabilistic linear vs distance-based). Topic structure used LDA and K-Means to triangulate latent themes and cluster separation. Aspect framing and temporal coupling were assessed to move beyond simple polarity into substantive narrative differences.

\textbf{Experimentation process (decision path).} The final configuration was chosen through explicit trial stages. First, we used snippet-only media text as a baseline for full corpus coverage and fast iteration. Second, we tested body-only media text to improve semantic depth, but coverage volatility made this representation brittle. Third, we compared \texttt{snippet}, \texttt{body}, and \texttt{hybrid} on the same frozen dataset and retained \texttt{hybrid} as the operating mode because it preserved body richness while controlling missing-body risk through fallback. For recollection, a strict run (timeout 15, min body tokens 80) failed representation gates, then a second run (timeout 25, min body tokens 60) passed and became the reporting snapshot.

\textbf{Hyperparameter tuning and robustness checks.} Classification used leakage-safe \texttt{Pipeline} + \texttt{StratifiedKFold} + \texttt{GridSearchCV}. Naive Bayes tuned $\alpha$ and KNN tuned neighborhood size $k$ inside cross-validation. Robustness was tested in two ways: (1) length-normalized reruns (token cap = 40) to reduce document-length confounding, and (2) a within-public Reddit-vs-WebMD subanalysis to verify that public discourse is itself heterogeneous rather than a single homogeneous class.
...
\subsection{Public Subanalysis: Reddit vs WebMD}
Within the public corpus, Reddit and WebMD remain distinct. Mean sentiment differs (Reddit 0.1479 vs WebMD -0.3077; t = 6.885, p = 0.000000, d = 0.702). Lexical overlap is moderate (cosine = 0.751), and source separability is high (NB = 0.9222, KNN = 0.9238 with k = 7). Reddit tends to emphasize terms such as weight, week, ozempic, dose, whereas WebMD emphasizes week, day, effect, side.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{../figures/reddit_webmd_classification.png}
\caption{Public subanalysis: Reddit vs WebMD classification performance.}
\end{figure}
