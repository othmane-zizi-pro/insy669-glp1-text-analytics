...
\textbf{Experimentation process.} The final analysis design came from staged trials rather than a single pass. We first ran snippet-based media text as the baseline to verify full coverage and stable end-to-end execution. We then tested body-only media text to increase semantic depth, but publisher-level extraction variability reduced representativeness. We therefore selected hybrid mode and finalized it after a strict run (timeout 15, minimum 80 tokens) failed coverage checks and a second run (timeout 25, minimum 60 tokens) passed the representation gates.
